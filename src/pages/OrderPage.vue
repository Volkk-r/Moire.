...
export default {
  ...
  methods: {
    // Функция, возвращающая данные о товарах в заказе
    orderListInfo () {
      return {
        items: { ...this.$store.state.cartProductsData },
    ...
    order () {
      console.log(this.formData, this.deliveryType, this.paymentType)
      this.formError = {}
      ...
      axios.post(API_BASE_URL + '/api/orders', {
        ...
        .catch(error => {
          ...
          if (error.response.data.error.request && error.response.data.error.request.paymentTypeId) {
